...
public class BarHorizontalChart extends BaseChart {
    ...
    private List<List<TimeBarBean>> dataList;
     ...
     */
    public void setData(List<List<TimeBarBean>> dataList, List<String> strYList){
        this.strList.clear();
        this.dataList.clear();
        if(dataList!=null)
    ...
    private void evaluatorByData(){
        ...
        /*计算X刻度最大值*/
        XMARK_MAX =  1;
        for(List<TimeBarBean> list : dataList){
            for(TimeBarBean bean : list){
                total += bean.getNum();
...
//        LogUtil.i(TAG, "leftStart："+leftStart+"   zezeroPoint"+zeroPoint+"   barItemSpace="+barItemSpace+"   barWidth="+barWidth +"   oneBarW="+oneBarW);
        ...
        for(int i = 0; i<dataList.size(); i++){
            ...
            List<TimeBarBean> list = dataList.get(i);
            //柱子开始绘制的Y坐标
